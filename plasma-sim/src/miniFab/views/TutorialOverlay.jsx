      ...
      case PHASE.INTRO:
        return (
          <>
            ...
            <div className="text-[12px] text-gray-400 leading-relaxed mb-2">{scenario.description}</div>
      ...
      case PHASE.PLAN:
        return (
          <>
            ...
            <ol className="text-[12px] text-gray-400 space-y-0.5 max-h-44 overflow-auto pr-1">
      ...
      case PHASE.STEP_LOAD:
        return (
          <div className="text-[13px] text-gray-300 leading-relaxed">
            ...
            <div className="text-[11px] text-gray-500 mt-1">실제 fab에서는 wafer를 cassette에서 chamber로 옮기는 단계입니다.</div>
          </div>
        );
      case PHASE.STEP_RUN:
        return (
          <div className="text-[13px] text-gray-300 leading-relaxed">
            ...
            {stepTutorialMsg && <div className="mt-1 text-[12px] text-cyan-200">💡 {stepTutorialMsg}</div>}
          </div>
        );
    ...
    <div className="fixed bottom-4 right-4 z-50 max-w-md w-[calc(100%-2rem)] sm:w-96 font-mono"
      ...
      <div className="border border-cyan-500/60 bg-[#0a1018]/95 backdrop-blur shadow-2xl"
        ...
        <div className="flex items-center gap-2 px-3 h-8 border-b border-cyan-700/40"
          ...
          <span className="text-[12px] tracking-[2px] text-cyan-300 uppercase">TUTORIAL</span>
          <span className="ml-auto text-[11px] tracking-[1px] text-gray-500 uppercase">
            {phase.replace('_', ' ')}
        ...
        {/* Body */}
        <div className="p-3">
          <div className="text-[11px] tracking-[2px] text-cyan-500 mb-1 uppercase">{titleByPhase[phase]}</div>
          {bodyByPhase()}
        </div>
        ...
        <div className="flex justify-between items-center px-3 h-9 border-t border-cyan-900/40 bg-[#080c12]">
          <button
            onClick={onSkip}
            className="text-[11px] tracking-[1.5px] text-gray-600 hover:text-rose-300 uppercase"
          >
            ✕ SKIP TUTORIAL
          </button>
          <button
            onClick={onAdvance}
            disabled={waiting}
            className={`px-3 py-1 text-[12px] tracking-[2px] uppercase border transition ${
              waiting
